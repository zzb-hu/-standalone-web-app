...
// Manages a temporary directory for staging local web files during packaging.
// Replaces the old dist_bak approach: instead of mutating the package's own
// dist/ directory, we copy user content into an OS temp directory and point
// Tauri's frontendDist there. The temp dir is cleaned up after the build.
let _stagedTempDir: tmp.DirectoryResult | null = null;

// Ensure tauriConf.build is an object we can write frontendDist into.
// Tests and some callers construct PakeTauriConfig without a `build` field.
function ensureBuildField(tauriConf: PakeTauriConfig): void {
  if (!tauriConf.build || typeof tauriConf.build !== 'object') {
    (tauriConf as { build: Record<string, unknown> }).build = {};
  }
}

async function stageLocalTree(sourceDir: string): Promise<string> {
  let resolvedSource: string;
  try {
    resolvedSource = await fsExtra.realpath(sourceDir);
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    throw new PakeError(
      `Local input "${sourceDir}" could not be resolved: ${detail}`,
      {
        code: 'INVALID_INPUT',
  ...
  // Create a temp directory for staging. If tmp.dir fails (e.g. permission
  // denied, no space), surface as BUILD_FAILED with a clear hint.
  let tempResult: tmp.DirectoryResult;
  try {
    tempResult = await tmp.dir({
      unsafeCleanup: true,
      prefix: 'pake-staging-',
    });
  } catch (error) {
    const detail = error instanceof Error ? error.message : String(error);
    throw new PakeError(
      `Failed to create a temp staging directory: ${detail}`,
      {
        code: 'BUILD_FAILED',
        hint: 'Check OS temp directory permissions and free space.',
      },
    );
  }
  const { path: tempDir, cleanup } = tempResult;
  _stagedTempDir = { path: tempDir, cleanup };

  // Copy user content into temp dir. On failure, clean up the temp dir
  // before propagating the error so we never leak staged files.
  try {
    fsExtra.copySync(resolvedSource, tempDir, {
      overwrite: true,
      dereference: true,
    });
  } catch (error) {
    // Best-effort cleanup; ignore errors here because we are already failing.
    try {
      await cleanup();
    } catch {
      /* swallow */
    ...
    const detail = error instanceof Error ? error.message : String(error);
    throw new PakeError(
      `Failed to stage local content from "${sourceDir}" to "${tempDir}": ${detail}`,
      {
        code: 'BUILD_FAILED',
        hint: 'Check that the source directory is readable and not blocked by another process.',
      },
    );
  }

  logger.debug(`✺ Staged local content to temp dir: ${tempDir}`);
  return tempDir;
}

// Clean up the temp directory created during local file staging.
// Safe to call even if no staging happened (no-op). The cleanup is async
// (tmp-promise returns a Promise<void>), but this function is sync to
// preserve the existing API: we fire-and-forget the cleanup and log any
// failure instead of letting an unhandled rejection propagate.
export function restoreLocalTree(): void {
  if (!_stagedTempDir) return;

  const { cleanup, path: dir } = _stagedTempDir;
  _stagedTempDir = null; // clear first so a re-entrant call is a no-op
  // Fire-and-forget the async cleanup; surface failures via logger only.
  cleanup().catch((error: unknown) => {
    const detail = error instanceof Error ? error.message : String(error);
    logger.warn(`Failed to clean temp staging dir ${dir}: ${detail}`);
  });
}
...
export async function handleLocalFile(
  ...
  if (stat.isDirectory()) {
    ...
    const stagedDir = await stageLocalTree(url);
    ensureBuildField(tauriConf);
    (tauriConf.build as Record<string, unknown>).frontendDist = stagedDir;
  ...
  if (!useLocalFile) {
    // Single-file mode: copy the file into the package's own dist/ directory.
    // ensureDir first: Task 3 removed dist/cli.js from git tracking, so a
    // fresh checkout may not have a dist/ directory at all.
    try {
      await fsExtra.ensureDir(distDir);
    } catch (error) {
      const detail = error instanceof Error ? error.message : String(error);
      throw new PakeError(
        `Failed to create package dist directory "${distDir}": ${detail}`,
        {
          code: 'BUILD_FAILED',
          hint: 'Check write permissions on the Pake CLI installation directory.',
        },
      );
    }
    ...
    try {
      ...
      const detail = error instanceof Error ? error.message : String(error);
      throw new PakeError(
        `Failed to copy local file "${url}" to "${urlPath}": ${detail}`,
        {
          code: 'BUILD_FAILED',
          hint: 'Check that the source file is readable.',
        },
      );
    }
  } else {
    const stagedDir = await stageLocalTree(path.dirname(url));
    ensureBuildField(tauriConf);
    (tauriConf.build as Record<string, unknown>).frontendDist = stagedDir;
  }
