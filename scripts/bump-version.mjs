...
const targets = [
  ...
  { label: 'src-tauri/tauri.conf.json', file: path.join('src-tauri', 'tauri.conf.json') },
];
...
try {
  ...
  cargoTomlRaw = fs.readFileSync(path.join(root, 'src-tauri', 'Cargo.toml'), 'utf8');
  cargoLockRaw = fs.readFileSync(path.join(root, 'src-tauri', 'Cargo.lock'), 'utf8');
  tauriConfRaw = fs.readFileSync(path.join(root, 'src-tauri', 'tauri.conf.json'), 'utf8');
} catch (error) {
...
const cargoLockVersionRegex = /(\[\[package\]\]\s+name = "pake"\s+version = )"[^"]+"/;
...
try {
  ...
  fs.writeFileSync(path.join(root, 'src-tauri', 'tauri.conf.json'), newTauriConfRaw);
} catch (error) {
  const detail = error instanceof Error ? error.message : String(error);
  console.error(
...
console.log('  1. pnpm run cli:build          # rebuild dist/cli.js (embeds version)');
